...
<br>OK. (- 이런식으로 식이 작성된 경우, 연산자 변경시 (%, %* 식으로 되는것을 operator_button함수내 해결
<br>OK. 과학적 표기로 되는 오류 equals_button() 내에서 string으로 처리하여 해결.
<br>OK. 괄호열기 버튼 이후 연산자 버튼 누를 시 (특히 %, \*, /) 를 막지 않고있음. 또한 나눗셈->퍼센트 연속 작성시 계속 퍼센트로 연속입력됨.
<br>OK. 3자리 미만은 =를 연속으로 눌러도 업데이트 되지 않으나, 자릿수 구분때문에 네자리 숫자부터는 =을 계속 눌러도 기록됨
